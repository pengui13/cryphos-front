    ...
    <div className="relative h-full w-full overflow-hidden rounded-3xl border border-white/10 bg-white/5">
      <AnimatePresence initial={false} mode="popLayout">
        ...
        >
          ...
          />
          <div className="absolute bg-[#b65fcf] mix-blend-multiply inset-0 bg-gradient-to-t from-[#0f1115]/70 via-transparent to-transparent" />
...
function Strength({ value = "" }) {
  ...
  const score = Math.min(1, (len > 7 ? 0.5 : len / 16) + (variety - 1) * 0.17); // 0..1
  return (
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // minimalist: use email as username
      await signUp({ username: email, email, password, password2: confirmPassword });
      setSnackbarData({ visible: true, status: "success", info: "Account created! Redirecting to login…" });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          ...
          >
            <header className="mb-8">
              <h1 className="text-[36px] font-extrabold tracking-tight">Create an account</h1>
            ...
            <form className="grid gap-7" onSubmit={handleSubmit}>
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-white/50">
                  ...
                  <Link href="/terms" className="text-[#e3b8ff] hover:underline">
                    Terms
                  </Link>{" "}
                  &{" "}
                  <Link href="/privacy" className="text-[#e3b8ff] hover:underline">
                    Privacy
                  </Link>
                  .
                </span>
